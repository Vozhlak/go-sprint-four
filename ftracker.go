...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	speed := meanSpeed(action, duration)     // Средняя скорость (м/с)
	speedInS := math.Pow(speed*kmhInMsec, 2) // Средняя скорость в м/с возведенная в квадрат
	heightInM := height / cmInM              // Перевод роста в метры
	return (walkingCaloriesWeightMultiplier*weight + (speedInS/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
